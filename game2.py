...
# Paths to images (update if needed)
image_paths = {
...
# Resize images (bigger icons for better visibility)
...
while running:
    ...
    while len(asteroids) < 3 + level:
        ...
        speed = random.uniform(1.8 + level * 0.3, 3.5 + level * 0.4)
        asteroids.append([x, y, speed])
    ...
    for pu in powerups[:]:
        pu[1] += 2
        img = images["shield"] if pu[2] == "shield" else images["double_score"] if pu[2] == "double_score" else images["rapid_fire"]
